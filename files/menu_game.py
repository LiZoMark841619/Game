...
class GameMenu(Menu):
    ...
    def chose_game(self) -> str:
        game = self.get_valid_str(f'Choose from {[cls.__name__ for cls in Game.__subclasses__()]}! ', 'guesses', 'lotto', 'rock')
        print(f'Welcome to my {game.title()} game! '); return game
    
    def want_to_quit(self) -> bool:
        if self.get_valid_str('Would you like to quit? Enter yes or no? ', 'yes', 'no') == 'yes': print('Understood, good bye!'); return True
